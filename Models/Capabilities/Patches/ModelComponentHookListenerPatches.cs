...
{
    /// <summary>
    ///     Inserts opt-in model-backed components into vanilla run/combat hook listener streams.
    ///     将 opt-in 的基于模型组件插入原版跑局/战斗 hook listener 流。
    /// </summary>
    internal static class ModelComponentHookListenerPatches
    {
        internal sealed class RunStateHookListenersPatch : IPatchMethod
        {
            public static string PatchId => "ritsulib_model_component_run_hook_listeners";

            public static string Description => "Insert model components into run hook listener streams";
            ...
            {
                __result = ModelComponentHookListeners.ExpandOwnerHookListeners(__result);
            }
        }

        internal sealed class CombatStateHookListenersPatch : IPatchMethod
        {
            public static string PatchId => "ritsulib_model_component_combat_hook_listeners";

            public static string Description => "Insert model components into combat hook listener streams";
