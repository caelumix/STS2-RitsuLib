...
namespace STS2RitsuLib.Scaffolding.Content
{
    ...
    ///     任意战斗生物模型（玩家角色和怪物）的运行时 <see cref="NCreatureVisuals" /> 工厂。请在模型类型上实现；
    ///     通常通过继承
    ///     <see cref="STS2RitsuLib.Scaffolding.Characters.ModCharacterTemplate{TCardPool,TRelicPool,TPotionPool}" />
    ///     或 <see cref="ModMonsterTemplate" /> 完成，不过这些模板只是便利封装，并非必需。非 null 的
    ///     <see cref="TryCreateCreatureVisuals" /> 会替换 <see cref="CharacterModel" /> 和
    ///     <see cref="MonsterModel" /> 上基于路径的 <c>CreateVisuals</c>。
    /// </summary>
    public interface IModCreatureVisualsFactory
    {
        /// <summary>
        ///     Combat visuals from code, or <c>null</c> to fall through to asset paths.
        ///     由代码创建的战斗视觉；返回 <c>null</c> 则继续使用资源路径。
        /// </summary>
    ...
    public interface IModMonsterCreatureVisualsFactory
    {
        ...
        ///     由代码创建的战斗视觉；返回 <c>null</c> 则使用资源路径。
        /// </summary>
        NCreatureVisuals? TryCreateCreatureVisuals();
    }
    ...
    ///     运行时遭遇战斗根 <see cref="Control" />。当使用工厂且没有 <c>CustomEncounterScenePath</c> 时，请设置
    ///     <see cref="SuppliesEncounterCombatSceneFromFactory" />，以保持 <c>HasScene</c> 正确。
    /// </summary>
    ...
    ///     供 <see cref="MegaCrit.Sts2.Core.Models.EventModel.CreateScene" /> 使用的运行时布局 <see cref="PackedScene" />。
    /// </summary>
    ...
    ///     供 <see cref="MegaCrit.Sts2.Core.Models.EventModel.CreateBackgroundScene" /> 使用的运行时背景
    ...
    ///     运行时事件 VFX <see cref="Node2D" />。当 VFX 由代码构建且磁盘上没有 VFX 场景文件时，使用
    ...
    ///     供 <c>OrbModel.CreateSprite</c> 使用的运行时充能球表现 <see cref="Node2D" />。如果其它系统依赖原版形态，
    ...
    ///     任意战斗生物模型（玩家角色和怪物）的运行时 Spine <see cref="CreatureAnimator" /> 工厂。它会覆盖默认原版
    ///     <c>GenerateAnimator</c>，让 mod 无需继承 <c>NCreature</c> 即可接入自定义 <see cref="AnimState" />
    ...
    ///     任意生物模型（玩家角色、怪物或其它 <see cref="AbstractModel" />）的运行时战斗
    ///     <see cref="ModAnimStateMachine" /> 工厂。适用于希望将 <see cref="NCreature.SetAnimationTrigger" /> 流程路由到
    ///     <see cref="ModAnimStateMachine.SetTrigger" />，而不是（或同时）走原版
    ///     <see cref="MegaCrit.Sts2.Core.Animation.CreatureAnimator" /> 路径的情况。它支持非 Spine 后端
    ///     （cue 帧、Godot <see cref="AnimationPlayer" />、<see cref="AnimatedSprite2D" />、组合动画），也支持构建在
    ...
    ///         常见实现会继承
    ///         <see cref="STS2RitsuLib.Scaffolding.Characters.ModCharacterTemplate{TCardPool,TRelicPool,TPotionPool}" />
    ///         或 <see cref="ModMonsterTemplate" />，但模板只是便利封装。契约本身通过接口 opt-in：任何实现此接口的模型类型
    ///         都会通过 <see cref="STS2RitsuLib.Scaffolding.Characters.Patches.ModCreatureCombatAnimationPlaybackPatch" />
    ///         路由，<b>不要求</b>继承模板。
    ///     </para>
    ///     <para>
    ...
    ///         <c>Revive</c>, …).
    ///         <see cref="ModAnimStateMachine.SetTrigger" /> 会收到与原版派发给 Spine animator 相同的 trigger 名称
    ...
    ///         <see cref="NCreature.SetAnimationTrigger" />。如果你依赖原版
    ///         <see cref="NCreature.StartReviveAnim" /> 的 gate，请保持
    ///         <see cref="MegaCrit.Sts2.Core.Animation.CreatureAnimator.HasTrigger" /> 中的 <c>Revive</c> 同步；
    ...
    public interface IModCreatureCombatAnimationStateMachineFactory
    {
        ...
        ///     <paramref name="visualsRoot" /> 为键的 <see cref="ConditionalWeakTable{TKey,TValue}" /> 缓存）。
        /// </summary>
