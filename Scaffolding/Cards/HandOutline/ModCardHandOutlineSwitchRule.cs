...
{
    /// <summary>
    ///     A hand-card outline rule whose resolver returns the outline color to apply, or <see langword="null" /> when the
    ///     rule does not match the current card state. This shape is intended for switch expressions and dynamic colors.
    ///     手牌卡牌描边规则：解析器返回要应用的描边颜色，当前卡牌状态不匹配时返回 <see langword="null" />。
        ...
        bool RefreshEveryFrame = true)
    {
        ...
        /// </summary>
        public static ModCardHandOutlineSwitchRule Switch<TCard>(
        ...
        /// </summary>
        public static ModCardHandOutlineSwitchRule Fixed<TCard>(
        ...
        {
            ArgumentNullException.ThrowIfNull(when);
            return Fixed(card => card is TCard typed && when(typed), color, priority, visibleWhenUnplayable);
        }
        ...
        /// </summary>
        public static ModCardHandOutlineSwitchRule Dynamic<TCard>(
            Func<TCard, bool> when,
            Func<TCard, Color> colorWhen,
            int priority = 0,
            bool visibleWhenUnplayable = false)
            where TCard : CardModel
        {
            ...
            ArgumentNullException.ThrowIfNull(colorWhen);
            return Dynamic(
                card => card is TCard typed && when(typed),
                card => colorWhen((TCard)card),
                priority,
                visibleWhenUnplayable);
        }
