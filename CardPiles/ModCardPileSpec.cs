...
namespace STS2RitsuLib.CardPiles
{
    ...
    ///     描述注册时的 mod card pile。除 mod id 和 local stem 外均为可选；
    ...
    ///     本地化遵循原版 pile 约定：hover-tip title / description 以及“打开空 pile”的 thought bubble
    ///     始终基于内置 <c>static_hover_tips</c> loc table 解析，使用 <c>"{id}.title"</c>、
    ///     <c>"{id}.description"</c> 和 <c>"{id}.empty"</c> key，其中 <c>id</c> 是已注册 pile id。
    ...
    public sealed record ModCardPileSpec
    {
        ...
        ///     构建带默认值的 spec，适用于仅战斗中存在、位于左下、向屏幕中心自动堆叠的 pile
        ///     （与 draw pile 同一行）。
        /// </summary>
        ...
        ///     pile 的生命周期 scope。默认 <c>ModCardPileScope.CombatOnly</c>。
        /// </summary>
        ...
        ///     视觉样式族；决定战斗中附加哪种 UI chrome。默认 <c>ModCardPileUiStyle.Headless</c>
        ///     （无 UI 按钮）。
        /// </summary>
        ...
        ///     与 <c>Style</c> 配对的 slot hint。保持为 <c>ModCardPileAnchor.Default</c> 时，
        ///     pile 会按注册顺序堆叠在其它同样式 pile 之后。
        /// </summary>
        ...
        ///     pile 按钮图标的 Godot ResourcePath（例如 <c>res://art/my_pile.png</c>）。为 null 或缺失时使用占位贴图。
        /// </summary>
        public string? IconPath { get; init; }

        /// <summary>
        ///     Optional controller / keyboard hotkey ids that open the pile's view screen.
        ///     打开 pile view screen 的可选手柄 / 键盘 hotkey id。
        /// </summary>
        ...
        ///     为 true 时，加入 pile 的 card 会在 pile 的 UI 容器中显示为 <c>NCard</c> 节点
        ///     （仅对 <c>ModCardPileUiStyle.ExtraHand</c> 有意义）。
        /// </summary>
        ...
        ///     添加到 hover tip 解析后 <c>Godot.Control.GlobalPosition</c> 的额外屏幕空间像素。
        ///     默认 <c>Vector2.Zero</c>。当 <c>ModCardPileAnchorKind.Custom</c> 的自动放置
        ///     需要微调时最有用。
        /// </summary>
        ...
        ///     hover tip 相对于 pile 按钮的锚定方式。默认 <c>ModCardPileHoverTipPlacement.Auto</c>。
        /// </summary>
        ...
        ///     非 null 时，会在 pile 按钮的 <c>_Process</c> tick 上定期求值（与
        ///     <c>TopBar.ModTopBarButtonSpec.VisibleWhen</c> 模式相同）。若 delegate 返回 false，
        ///     按钮会隐藏、忽略鼠标输入，并移除所有活跃 hover tip。为 null 时按钮始终显示
        ///     （仍受普通父节点可见性影响）。attribute-driven 注册不能提供 delegate；需要此功能时请从代码调用
        ...
        ///     do nothing at all.
        ///     pile 的 UI 按钮释放时调用的可选回调。为 null（默认）时，按钮会回退到
        ///     <c>NCardPileScreen.ShowScreen</c>，即与原版 Draw / Discard / Exhaust 按钮相同的行为。
        ///     提供 delegate 可接入自定义 <c>MegaCrit.Sts2.Core.Nodes.Screens.Capstones.ICapstoneScreen</c>、
        ///     检查 pile，或完全不做任何事。
        /// </summary>
        /// <remarks>
        ...
        ///     context 暴露 helper：<c>ModCardPileOpenContext.ShowDefaultPileScreen</c> 运行默认行为，
        ///     <c>ModCardPileOpenContext.OpenCapstoneScreen</c> 通过 <c>NCapstoneContainer</c> 挂载自定义画面。
        ///     pile 为空时不会调用此回调；此时会显示空 pile thought bubble，并且再次点击已打开的默认 pile screen
        ///     会继续先将其关闭。
        /// </remarks>
        ...
        ///     每次 card fly-in 到此 pile 时调用的可选 resolver，允许 mod 为 tail/trail endpoint 提供动态目标位置。
        ///     返回 null 表示使用默认布局位置。
        /// </summary>
        ...
        ///     shuffle 风格 fly visual 从此 pile 开始时调用的可选 resolver，允许 mod 提供动态 source/start 位置。
        ///     返回 null 表示使用默认布局位置。
        /// </summary>
