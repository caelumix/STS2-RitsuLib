...
namespace STS2RitsuLib.Scaffolding.Content
{
    ...
    ///     Mod 遭遇的基础 <see cref="EncounterModel" />：提供 <see cref="IModEncounterAssetOverrides" />（战斗场景路径、
    ///     背景、Boss 节点、地图节点预加载、额外路径）以及可选的 <see cref="TryCreateEncounterCombatScene" />。
    ///     背景管线匹配原版 <c>EncounterModel.HasCustomBackground</c> 语义，并提供显式开关以便需要时继续使用所属
    ///     act 的战斗背景。若想使用不依赖磁盘文件的背景，请设置 <see cref="UseProgrammaticCombatBackground" /> 并用
    ///     <see cref="CombatBackgroundAssetsFactory" /> 实现 <see cref="BuildProgrammaticCombatBackground" />（或复用
    ...
    ///     <see cref="MonsterModel" /> 都应通过 <c>RegisterMonster&lt;T&gt;()</c> / <c>Monster&lt;T&gt;()</c> 注册，以便
        ...
        IModEncounterCombatSceneFactory
    {
        ...
        ///     <see cref="ContentAssetProfiles.Encounter(string, string?, string?)" /> 的 profile 路径不会参与背景选择
        ///     （但仍会按需预加载遭遇场景 / 地图美术）。为 <c>false</c> 时，像原版 <c>HasCustomBackground</c> 一样使用来自
        ///     <see cref="AssetProfile" /> 的遭遇专属图层 / 主场景。
        /// </summary>
        ...
        ///     当 <see cref="UseProgrammaticCombatBackground" /> 为 <c>true</c> 时构建战斗背景资源。返回 <c>null</c> 会回退到
