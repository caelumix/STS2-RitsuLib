...
namespace STS2RitsuLib.Settings
{
    ...
    ///     标识一个按 Mod id、数据键和 <see cref="SaveScope" /> 存储的 Mod 设置值。
    /// </summary>
    ...
    ///     单个 <typeparamref name="TValue" /> 类型设置值的读写绑定。
    /// </summary>
    ...
    public interface IStructuredModSettingsValueAdapter<TValue>
    {
        ...
        ///     将 <paramref name="value" /> 序列化为单个文本块（例如 JSON）。
        /// </summary>
        string Serialize(TValue value);

        /// <summary>
        ///     Parses <paramref name="text" /> into <paramref name="value" />.
        ///     将 <paramref name="text" /> 解析到 <paramref name="value" />。
        /// </summary>
