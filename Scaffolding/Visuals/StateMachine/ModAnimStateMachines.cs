...
namespace STS2RitsuLib.Scaffolding.Visuals.StateMachine
{
    ...
    ///     但可用于任何 <see cref="IAnimationBackend" />（Spine、Godot animation player / animated sprite、cue 帧序列）。
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    ///         animated sprite).
    ///         <see cref="StandardCue" /> 会为以 <see cref="Node" /> 为根的非 Spine 视觉生成与后端无关的
    ///         <see cref="ModAnimStateMachine" />（cue 帧序列、Godot animation player、animated sprite）。
    ///     </para>
    ///     <para>
    ...
    public static class ModAnimStateMachines
    {
        ...
        ///     构建一个原版 Spine <see cref="CreatureAnimator" />，匹配标准 idle / dead / hit / attack / cast /
        ...
        ///     在 <paramref name="visualsRoot" /> 上构建非 Spine <see cref="ModAnimStateMachine" />，匹配标准 idle /
