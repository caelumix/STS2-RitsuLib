...
namespace STS2RitsuLib.Scaffolding.Content
{
    ...
    ///     the pack builder.
    ///     Mod 怪物的基础 <see cref="MonsterModel" />：<see cref="IModMonsterAssetOverrides" /> 提供视觉场景路径；
    ///     也可以重写 <see cref="TryCreateCreatureVisuals" /> 直接用代码构建 <see cref="NCreatureVisuals" />。
    ///     如需显式 <c>CreateFromResource</c> / <c>CreateFromScenePath</c> 构造，请使用
    ///     <see cref="RitsuGodotNodeFactories" />。请通过 <c>ModContentRegistry.RegisterMonster&lt;T&gt;()</c> 或 pack builder 上的
    ///     <c>Monster&lt;T&gt;()</c> 注册。
    /// </summary>
    /// <remarks>
    ...
    ///     当怪物视觉需要使用 <see cref="ModAnimStateMachine" /> 响应战斗触发时，请重写
    ///     <see cref="SetupCustomCombatAnimationStateMachine" /> 来驱动该生物；这与
    ///     <see cref="STS2RitsuLib.Scaffolding.Characters.ModCharacterTemplate{TCardPool,TRelicPool,TPotionPool}" />
    ///     使用的是同一套状态机管线。
    /// </remarks>
